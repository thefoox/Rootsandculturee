...
**Goal**: Fiks gjenvaerende medium-prioritet CMS-funn fra revisjon — useEffect feilhandtering, dupliserte mappere, slug-validering, hardkodede farger, cache-strategi og kodekvalitet
**Requirements**: CMS-MED-ERRORHANDLING, CMS-MED-MAPPERS, CMS-MED-SLUGS, CMS-MED-COLORS, CMS-MED-CACHE, CMS-MED-QUALITY
**Depends on**: Phase 18
**Success Criteria** (what must be TRUE):
  1. Alle admin useEffect-kall har .catch() med norsk feilmelding
  2. Mapper-funksjoner er deduplisert — en kilde per innholdstype
  3. Slug-oppretting validerer unikhet mot Firestore for lagring
  4. Ingen hardkodede hex-farger i admin-sider — alle bruker design-tokens
  5. Cache-strategi er konsistent (noStore ELLER revalidateTag, ikke begge)
  6. Konsistente ActionResult-typer pa alle server actions
**Plans**: 3 plans
Plans:
- [ ] 19-01-PLAN.md — ActionResult type + mapper deduplication (shared mappers in src/lib/mappers/)
- [ ] 19-02-PLAN.md — useEffect .catch() error handling + hardcoded hex colors replaced with design tokens
- [ ] 19-03-PLAN.md — Slug uniqueness validation + cache cleanup + section defaults + SiteContent removal
